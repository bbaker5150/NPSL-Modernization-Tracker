...
function Attention({ projects, onOpen }) {
  ...
  return <div className="page-stack"><section className="page-heading"><h1>Needs attention</h1><p>Outstanding tasks by status. Collapse a section to focus on the work that matters to you.</p></section><div className="attention-groups">{statuses.map((status) => {
    ...
    return items.length > 0 && <details className="panel attention-group" key={status} open><summary><Icon name="chevron" size={16} /><strong>{status}</strong><Badge tone={statusTone(status)}>{items.length}</Badge></summary><div>{items.map(({ project, task }) => <article className="attention-task" key={task.id}>
...
function ProjectDrawer({ project, user, manager, users, onDeleteTask, onProgressMode, tasks, updates, risks, onClose, onEdit, onSaveTask, onToggleTask, onSetPhaseRequired, onAddUpdate, onAddRisk, onDelete }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="project-drawer" role="dialog" aria-modal="true"><header className="drawer-header"><div className="drawer-title-row"><div><span className="section-kicker">{project.measurementArea}</span><h2>{project.title}</h2><div className="drawer-badges"><Badge tone={statusTone(project.health)} dot>{project.health}</Badge><Badge>{project.priority} priority</Badge><Badge>{project.status}</Badge></div></div><div className="drawer-header-actions">{manager && <div className="project-menu-wrap"><button className="icon-button" aria-label="Project settings" aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}><Icon name="more" className="vertical-more" /></button>{menuOpen && <div className="project-menu-popover"><button onClick={() => { setMenuOpen(false); onEdit(project); }}>Edit project</button><button onClick={() => { setMenuOpen(false); onDelete(project); }}>Delete project</button></div>}</div>}<button className="icon-button" onClick={onClose} aria-label="Close"><Icon name="close" /></button></div></div>
    ...
    <div className={`mini-pipeline ${progressMode === 'tasks' ? 'task-markers' : ''}`} aria-label={progressMode === 'tasks' ? 'Task progress markers' : 'Phase progress markers'}>{markers.map((item, index) => <span key={item.key} className={item.done ? 'done' : item.current ? 'current' : ''} title={item.label} aria-label={item.label} aria-current={item.current && !item.done ? 'step' : undefined}>{index + 1}</span>)}</div>
