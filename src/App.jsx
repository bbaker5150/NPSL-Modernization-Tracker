...
function enrichProject(project, allTasks) {
  ...
  return {
    ...project,
    tasks,
    health: tasks.some((task) => task.status === 'In Progress – At Program Office') ? 'Blocked' : project.health,
    ...
    blockedCount: tasks.filter((task) => ['Blocked', 'In Progress – At Program Office'].includes(task.status)).length,
...
function Overview({ projects, tasks, onOpen, phaseFilter, setPhaseFilter, onAttention, onBoard }) {
...
function Attention({ projects, onOpen }) {
  const rows = projects.filter((project) => project.status !== 'Complete').flatMap((project) => project.tasks.filter((task) => !isClosedTask(task)).map((task) => ({ project, task })));
  ...
  return <div className="page-stack"><section className="page-heading"><h1>Needs attention</h1><p>Outstanding tasks by status. Collapse a section to focus on the work that matters to you.</p></section><div className="attention-groups">{statuses.map((status) => {
    ...
    return items.length > 0 && <details className="panel attention-group" key={status} open><summary><Icon name="chevron" size={16} /><Badge tone={statusTone(status)}>{status}</Badge><Badge tone={statusTone(status)}>{items.length}</Badge></summary><div>{items.map(({ project, task }) => <article className="attention-task" key={task.id}>
      <button className="text-button" onClick={() => onOpen(project)}>{project.title}</button><h3>{task.title}</h3><p>Task owner: {task.ownerName || project.ownerName || 'Unassigned'}</p>
...
function ProjectsTable({ projects, onOpen }) {
  const [progressMode, setProgressMode] = useState('phases');
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell"><header className="topbar"><button className="mobile-brand" aria-label="Portfolio home" onClick={() => { setOpenProjectId(''); setView('overview'); }}><img className="brand-logo" src={navairSeal} alt="NAVAIR" /><strong>MODERNIZATION</strong></button><div className="top-actions"><button className="button secondary export-button" disabled={exporting} onClick={exportWorkbook}><Icon name="download" /> {exporting ? 'Building Excel…' : 'Export Excel'}</button>{manager && <button className="button primary" onClick={() => setProjectEditor({})}><Icon name="plus" /> New project</button>}<button className="icon-button" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}><Icon name={theme === 'dark' ? 'sun' : 'moon'} /></button></div></header>
      <main>
        {view === 'overview' && <Overview projects={enriched} tasks={data.tasks} risks={data.risks} onOpen={(project) => setOpenProjectId(project.id)} onDelete={manager ? deleteProject : undefined} phaseFilter={phaseFilter} setPhaseFilter={setPhaseFilter} onAttention={() => setView('attention')} onBoard={() => setView('board')} />}
