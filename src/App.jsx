...
const TASK_STATUSES = ['Not Started', 'In Progress', 'In Progress – At Program Office', 'Blocked', 'Complete', 'Not Required'];
  ...
  return <div className="pipeline-rail">
    {workflowData.phases.map((phase, index) => {
      ...
      return <button key={phase.key} className={`phase-node ${selected.includes(phase.key) ? 'selected' : ''}`} aria-pressed={selected.includes(phase.key)} onClick={(event) => onSelect?.(event.ctrlKey || event.metaKey ? (selected.includes(phase.key) ? selected.filter((key) => key !== phase.key) : [...selected, phase.key]) : (selected.length === 1 && selected[0] === phase.key ? [] : [phase.key]))}>
        ...
        <span className="phase-count" aria-label={`${count} projects`}>{count}</span>
...
function ProjectCard({ project, onOpen, onDelete }) {
  const { menuOpen, setMenuOpen, menuRef } = useDismissibleMenu();
  ...
  return <article className="project-card" role="button" tabIndex="0" onClick={openProject} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openProject(); } }}>
    <div className="project-card-top"><Badge tone={statusTone(project.health)} dot>{project.health}</Badge><div ref={menuRef} className="project-menu-wrap" hidden={!onDelete}><button className="project-menu" aria-label={`Project actions for ${project.title}`} aria-expanded={menuOpen} onClick={(event) => { event.stopPropagation(); setMenuOpen((open) => !open); }}><Icon name="more" size={16} /></button>{menuOpen && onDelete && <div className="project-menu-popover"><button onClick={(event) => { event.stopPropagation(); setMenuOpen(false); onDelete(project); }}>Delete project</button></div>}</div></div>
    <div><span className="eyebrow">{project.measurementArea}</span><h3>{project.title}</h3></div>
    <div className="project-owner"><span className="avatar small">{project.ownerName === 'Unassigned' ? '?' : project.ownerName.slice(0, 1)}</span><span>{displayName(project.ownerName)}</span></div>
...
function Overview({ projects, tasks, onOpen, phaseFilter, setPhaseFilter, onAttention, onBoard, register, setRegister }) {
  const active = projects.filter((project) => project.tasks.some((task) => !isClosedTask(task)) || !project.tasks.length);
  ...
  return <div className="page-stack">
    ...
    <section className="kpi-grid">
      ...
      <KpiCard icon="alert" label="Needs attention" value={projects.filter((project) => project.tasks.some((task) => !isClosedTask(task))).length} detail="Review outstanding tasks and active deferrals" tone="amber" onClick={onAttention} />
    </section>
...
function ProgressChoice({ value, onChange, label = 'Progress calculation' }) {
  return <label className="progress-choice" title={`Progress by ${value}`}><Icon name="chevron" size={15} /><select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)}><option value="phases">By phase</option><option value="tasks">By task</option></select></label>;
}
...
function ProjectsTable({ projects, onOpen, register, setRegister }) {
  const { progressMode, health, search } = register;
  const setProgressMode = (progressMode) => setRegister((state) => ({ ...state, progressMode }));
  const setHealth = (health) => setRegister((state) => ({ ...state, health }));
  const setSearch = (search) => setRegister((state) => ({ ...state, search }));
  const query = search.trim().toLowerCase();
  const rows = projects.filter((project) => (!health || project.health === health) && (!query || [project.title, project.measurementArea, project.ownerName, displayName(project.ownerName), project.nextMilestone, project.health].some((value) => String(value || '').toLowerCase().includes(query))));
...
function ProjectEditor({ project, user, users, onClose, onSave }) {
  ...
  return <Modal title={project?.id ? 'Edit project' : 'Add modernization project'} subtitle="Update the portfolio record and ownership." onClose={onClose} actions={<><button className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={!draft.title.trim()} onClick={() => onSave({ ...draft, id: draft.id || uid('project'), projectKey: draft.projectKey || uid('project-key'), percentComplete: draft.percentComplete || 0, nextMilestone: draft.nextMilestone || 'Define project plan', nextMilestoneDate: draft.nextMilestoneDate || '' })}>Save project</button></>}>
    <UserPicker users={users} label="Assign project owner" onSelect={(entry) => setDraft((row) => ({ ...row, ownerName: entry.title, ownerEmail: entry.email, ownerKey: userIdentityKey(entry) }))} /><UserPicker users={users.filter((entry) => entry.role === 'Manager')} label="Project manager" onSelect={(entry) => setDraft((row) => ({ ...row, managerName: entry.title, managerEmail: entry.email || entry.loginName }))} /><p>Manager: {displayName(draft.managerName || 'Unassigned')}</p><div className="field-grid"><Field label="Project name"><input value={draft.title} onChange={set('title')} /></Field><Field label="Measurement area"><input value={draft.measurementArea} onChange={set('measurementArea')} /></Field><Field label="Project owner"><input value={draft.ownerName || ''} placeholder="Unassigned" onChange={set('ownerName')} /></Field><Field label="Owner email"><div className="input-action"><input type="email" value={draft.ownerEmail || ''} placeholder="Unassigned" onChange={setOwnerIdentity} /><button type="button" onClick={claim}>Assign me</button></div></Field><Field label="Priority"><select value={draft.priority} onChange={set('priority')}>{PRIORITIES.map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="Health"><select value={draft.health} onChange={set('health')}>{HEALTHS.map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="Current stage"><select value={draft.currentStageKey} onChange={set('currentStageKey')}>{workflowData.phases.map((phase) => <option value={phase.key} key={phase.key}>{phase.name}</option>)}</select></Field><Field label="Target completion"><input type="date" value={draft.targetFinish || ''} onChange={set('targetFinish')} /></Field><Field label="Description" wide><textarea rows="4" value={draft.description} onChange={set('description')} /></Field></div>
  </Modal>;
}
...
function UserDirectory({ users, manager, testingEnabled, onActivate, onSave }) {
  ...
  return <section className="page-stack"><h1>Users and managers</h1><p>Signed-in users and their application roles.</p>{!manager && testingEnabled && <form className="panel directory-form" onSubmit={activate}><h2>Testing manager access</h2><p>Testing only: enter the shared testing password to grant your signed-in account manager access.</p><Field label="Testing password"><input type="password" autoComplete="off" value={password} onChange={(event) => setPassword(event.target.value)} /></Field>{activationError && <p role="alert" className="inline-error">{activationError}</p>}<button type="button" className="button primary" onClick={activate} disabled={saving || !password}>{saving ? 'Saving and verifying role…' : 'Enable manager access'}</button>{saving && <p role="status">Please wait while your manager role is saved and verified.</p>}</form>}{manager && <form className="panel directory-form" onSubmit={async (event) => { event.preventDefault(); setSaving(true); try { if (await onSave({ ...draft, id: draft.id || uid('user'), loginName: draft.loginName?.trim() || draft.email.trim() })) setDraft(empty); } finally { setSaving(false); } }}>
    {['title', 'email'].map((key) => <Field key={key} label={{ title: 'Name', email: 'Email', loginName: 'SharePoint login' }[key]}><input type={key === 'email' ? 'email' : 'text'} required value={draft[key]} onChange={(event) => setDraft((row) => ({ ...row, [key]: event.target.value }))} /></Field>)}
    <Field label="Role"><select value={draft.role} onChange={(event) => setDraft((row) => ({ ...row, role: event.target.value }))}><option>User</option><option>Manager</option></select></Field><button className="button primary" disabled={saving || !draft.email.trim()}>Save user</button>{draft.id && <button type="button" className="button secondary" onClick={() => setDraft(empty)}>Cancel edit</button>}
    </form>}{users.map((entry) => <article className="panel directory-row" key={entry.id}><strong>{displayName(entry.title)}</strong><span>{entry.email || 'Email not available'}</span><Badge>{entry.role}</Badge>{manager && <div className="directory-actions"><button className="button secondary" onClick={() => setDraft(entry)}>Edit</button>{entry.role !== 'Manager' && <button className="button secondary" disabled={saving} onClick={async () => { setSaving(true); try { await onSave({ ...entry, role: 'Manager' }); } finally { setSaving(false); } }}>Make manager</button>}</div>}</article>)}</section>;
}

function ProjectDrawer({ project, user, manager, users, onDeleteTask, onProgressMode, tasks, updates, risks, onClose, onEdit, onSaveTask, onToggleTask, onSetPhaseRequired, onAddUpdate, onAddRisk, onDelete }) {
  const [tab, setTab] = useState('overview');
  const ownsProject = isOwnedByUser(project, user);
  const { menuOpen, setMenuOpen, menuRef } = useDismissibleMenu();
  const [progressMode, setProgressMode] = useState(project.progressMode || 'phases');
  const [taskEditor, setTaskEditor] = useState(null);
  const [updateText, setUpdateText] = useState('');
  const [riskDraft, setRiskDraft] = useState({ title: '', severity: 'Medium', probability: 'Possible', mitigation: '' });
  const projectTasks = tasks.filter((task) => task.projectKey === project.projectKey).sort((a, b) => phaseIndex(a.phaseKey) - phaseIndex(b.phaseKey) || (a.order || 0) - (b.order || 0));
  const progress = projectProgress(projectTasks, progressMode);
  const currentTask = projectTasks.find((task) => !isClosedTask(task));
  const markers = progressMode === 'tasks' ? projectTasks.map((task) => ({ key: task.id, label: `${task.title} — ${task.status}`, done: task.status === 'Complete', current: task.id === currentTask?.id })) : workflowData.phases.map((item) => { const rows = projectTasks.filter((task) => task.phaseKey === item.key); return { key: item.key, label: item.name, done: rows.length > 0 && rows.every(isClosedTask), current: item.key === project.currentStageKey }; });
  ...
  const phase = workflowData.phases.find((item) => item.key === project.currentStageKey);
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="project-drawer" role="dialog" aria-modal="true"><header className="drawer-header"><div className="drawer-title-row"><div><span className="section-kicker">{project.measurementArea}</span><h2>{project.title}</h2><div className="drawer-badges"><Badge tone={statusTone(project.health)} dot>{project.health}</Badge><Badge>{project.priority} priority</Badge></div></div><div className="drawer-header-actions">{manager && <div ref={menuRef} className="project-menu-wrap"><button className="icon-button" aria-label="Project settings" aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}><Icon name="more" className="vertical-more" /></button>{menuOpen && <div className="project-menu-popover"><button onClick={() => { setMenuOpen(false); onEdit(project); }}>Edit project</button><button onClick={() => { setMenuOpen(false); onDelete(project); }}>Delete project</button></div>}</div>}<button className="icon-button" onClick={onClose} aria-label="Close"><Icon name="close" /></button></div></div>
    <div className="drawer-progress"><div className="progress-caption"><span>{progress.percentComplete}% complete</span><strong>{progressMode === 'tasks' ? (currentTask?.title || (projectTasks.length ? 'No outstanding tasks' : 'No tasks yet')) : phase?.name}</strong><ProgressChoice value={progressMode} onChange={(mode) => { setProgressMode(mode); if (manager || ownsProject) onProgressMode(project.id, mode); }} /></div><Progress value={progress.percentComplete} /><p className="progress-detail">{progress.progressDetail}</p></div>
...
export function App() {
  const [register, setRegister] = useState({ progressMode: 'phases', health: '', search: '' });
  ...
  async function exportWorkbook() {
    ...
    try {
      ...
      if (view === 'attention') scope = scope.filter((project) => project.tasks.some((task) => !isClosedTask(task)));
  ...
  return <div className="app-shell">
    <aside className="sidebar"><button className="brand" aria-label="Portfolio home" onClick={() => { setOpenProjectId(''); setView('overview'); }}><img className="brand-logo" src={navairSeal} alt="NAVAIR" /><div><strong>MODERNIZATION</strong><span>Project Tracker</span></div></button><nav>{[...NAV.filter(([key]) => manager || ['my-work', 'glossary'].includes(key)), ['users', 'Users and managers', 'tasks']].map(([key, label, icon]) => <button className={view === key ? 'active' : ''} key={key} onClick={() => setView(key)}><Icon name={icon} /><span>{label}</span>{key === 'my-work' && <Badge>{enriched.filter((project) => isOwnedByUser(project, user)).length}</Badge>}</button>)}</nav><div className="sidebar-footer"><div className="sidebar-user"><span className="avatar">{userInitials(user)}</span><div><strong>{displayName(user?.title || 'SharePoint user')}</strong><span>{user?.email || 'Email not available'}</span></div></div></div></aside>
    <div className="main-shell"><header className="topbar"><button className="mobile-brand" aria-label="Portfolio home" onClick={() => { setOpenProjectId(''); setView('overview'); }}><img className="brand-logo" src={navairSeal} alt="NAVAIR" /><strong>MODERNIZATION</strong></button><div className="top-actions"><button className="button secondary export-button" disabled={exporting} onClick={exportWorkbook}><Icon name="download" /> {exporting ? 'Building Excel…' : 'Export Excel'}</button>{manager && <button className="button primary" onClick={() => setProjectEditor({})}><Icon name="plus" /> New project</button>}<button className="icon-button" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}><Icon name={theme === 'dark' ? 'sun' : 'moon'} /></button></div></header>
      <main>
