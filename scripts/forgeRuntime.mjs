...
export function applyForgeRuntime(html, {
  ...
  const stamp = build
    ? `<meta name="x-modernization-tracker-build" content="${String(build).replace(/"/g, '&quot;')}" />\n`
      + `<script>window.__MOD_TRACKER_BUILD__=${JSON.stringify(String(build))};</script>\n`
    : '';
 ...
 */
export function forgeRuntime({ project = 'Uncertainty Budget', build } = {}) {
  return {
    name: 'forge-runtime',
    enforce: 'post',
